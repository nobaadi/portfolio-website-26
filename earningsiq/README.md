...
A retrieval-augmented generation system for querying annual reports. Select a document or upload your own PDF, ask a question in plain English, and get a grounded answer citing the specific passages it was drawn from.
...
[![Llama](https://img.shields.io/badge/Llama-3.1_8B-blueviolet)](https://groq.com)
...
4. **Generate** -- retrieved passages are injected as context into a Llama 3.1 8B prompt via Groq; the model generates a grounded answer and is instructed to cite passage numbers

The frontend calls `/api/query` with the question, document ID, and retrieval mode. No API key is required from the user -- generation runs server-side. A BM25/TF-IDF toggle in the sidebar switches retrieval algorithms without reloading the document. The `/api/compare` endpoint runs both algorithms on the same query without generation -- useful for seeing exactly where they agree and diverge.

Demo documents (DBS Group 2023 and Singapore Airlines FY2022/23 annual report excerpts) are auto-ingested at startup. Users can also upload their own PDF or TXT files via the sidebar.
...
Set `GROQ_API_KEY` in your environment before starting -- the backend uses it for answer generation. Get a free key at [console.groq.com](https://console.groq.com) (no credit card required).
...
GROQ_API_KEY=your_key uvicorn app.main:app --reload --port 8003
```
...
|--------|----------|-------------|
| POST | `/api/query` | Query with generation. Body: `{document_id, query, retrieval_mode}` where `retrieval_mode` is `"bm25"` (default) or `"tfidf"` |
| POST | `/api/compare` | Run both TF-IDF and BM25 on the same query, return retrieved passages and agreement rate. Body: `{document_id, query}` |
| POST | `/api/upload` | Upload and index a PDF or TXT file. Multipart form: `file` (required), `title` (optional) |
| POST | `/api/ingest` | Index a document from raw text. Body: `{document_id, text, title}` |
...
curl -X POST http://localhost:8003/api/query \
  ...
  -d '{
    ...
    "query": "What drove revenue growth this year?",
    "retrieval_mode": "bm25"
  }'
```
...
curl -X POST http://localhost:8003/api/upload \
  ...
  -F "title=My Company 2024 Annual Report"
```

### Compare retrieval example
...
│   └── Dockerfile
├── frontend/
│   └── index.html               # Chat interface with BM25/TF-IDF toggle and PDF upload (vanilla JS)
├── docker-compose.yml
└── README.md
```
